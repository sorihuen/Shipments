...
export class RouteRepository {
  ...
  async addDriverToRoute(routeId: number, driverId: number): Promise<void> {
    ...
    await AppDataSource.getRepository(Driver).update(driverId, {
      isAvailable: false,
    });
  }
  ...
  /**
   * Busca una ruta por condiciones específicas.
   */
  async findOneBy(options: any): Promise<Route | null> {
    return await this.routeRepository.findOneBy(options);
  }
  /**
 * Busca una ruta por su ID y carga los conductores asociados.
 */
async findOneWithDrivers(routeId: number): Promise<Route | null> {
    return await AppDataSource.getRepository(Route)
        .createQueryBuilder("route")
        .leftJoinAndSelect("route.drivers", "driver") // Cargar la relación many-to-many
